...
const wristAction = throttle(function handleWristAction({ x, y }) {
  if (
    pointInCircle(
      { x, y },
      { x: soundImgPos.x + 64, y: soundImgPos.y + 64 },
  ...
  if (pointInCircle({ x, y }, { x: upImgPos.x + 64, y: upImgPos.y + 64 }, 64)) {
    angleThreshold++;
  }
  if (
    pointInCircle({ x, y }, { x: downImgPos.x + 64, y: downImgPos.y + 64 }, 64)
  ) {
    angleThreshold--;
  }
}, 500);
